...
def isolation_forest(my_data, int_count):
    ...
    aa, bb = np.meshgrid(np.linspace(np.min(my_data[:, 1])-1, 15, 50),
                         np.linspace(np.min(my_data[:, 0])-1, 15, 50))
    ...
    c = plt.scatter(X_test[anom_test, 0], X_test[anom_test, 1], c='red',
                    s=25, edgecolor='k')
    plt.axis('tight')
    plt.xlim((np.min(my_data[:, 1])-1, 15))
    plt.ylim((np.min(my_data[:, 0])-1, 15))
    plt.legend([a, b, c],
