...
def extract_features(vibrationData, int_counter, windowSize): 
    ...
    for i in range(nwindows):
        ...
        fvl1 = []
        window = vibrationData[s_idx:s_idx + windowSize]
        s_idx = s_idx + round(windowSize/1.25)               # start index for i-th image
        xavp = sum(power(window, 2))/windowSize                                           # Average power
        xrms = math.sqrt(int(sum((power(window,2))))/windowSize)                              # RMS
        xsra = math.pow((sum(sqroot(absol(window)))/windowSize),2)                          #Square  root of amplitude
        xm = np.mean(window);                                                                   # Mean value
        sd = np.std(window);   
        xkv = sum(power(((window-xm)/sd),4))/windowSize                                  # Kurtosis value
        xsv = sum(power(((window-xm)/sd),3))/windowSize 
        #print("xkv value:", xkv)                                   # Skewness value
        maxW = max(window)                             
        minW = min(window)
        xppv = maxW-minW                                                           # Peak to Peak value
        xcf = max(absol(window))/math.sqrt((sum(power(window,2))/windowSize))                   # Crest factor
        xif = max(absol(window))/(sum(absol(window))/windowSize)                          # Impulse factor
        xmf = max(absol(window))/(math.pow((sum(sqroot(absol(window)))/windowSize), 2))            # Margin factor
        xsf = math.sqrt(sum(power(window,2))/(windowSize))/(sum(absol(window))/windowSize)          # Shape factor
        xkf = 10 #(power(sum(((window-xm))/sd), 4)/windowSize)/(power((sum(power(window,2))/windowSize),2))#Kurtosis factor
